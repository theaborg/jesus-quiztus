...
import { getGameDetails } from '../api/game/getGameDetails';
import { setGameState } from '../api/game/setGameState';
    ...
    const loadGameDetails = async () => {
      try {
        ...
        if (data.state === "active") {
          const formatted = await getQuestions(data.question_set);
      ...
      .on(
        ...
        async (payload) => {
          ...
          if (newState === "active" && prevGameStateRef.current !== "active") {
            const formatted = await getQuestions(
      ...
      interval = setInterval(async () => {
        ...
        if (remaining <= 0) {
          ...
          if (
            ...
            if (hostId === userId) {
              await setGameState(gameId, "over");
            }
          }
        }
      }, 100);
