...
var updateUserAvatar = async (supabaseClient, userId, file, corsHeaders2) => {
  ...
  const { error: uploadError } = await supabaseClient.storage.from("profile-pictures").upload(filePath, file.stream(), {
    cacheControl: "3600",
    upsert: true,
  ...
  const { data: userData, error: fetchError } = await supabaseClient.from("users").select("profile_picture").eq("id", userId).single();
  if (fetchError) {
  ...
  if (userData?.profile_picture) {
    const { data: deleteData, error: deleteError } = await supabaseClient.storage.from("profile-pictures").remove([`${userId}/${userData.profile_picture}`]);
    if (deleteError) {
      console.error("Delete failed:", deleteError.message);
    }
  }
  const { error: dbError } = await supabaseClient.from("users").update({ profile_picture: `profile.${currentDate}` }).eq("id", userId);
  if (dbError) {
  ...
  const { data: urlData, error: urlError } = supabaseClient.storage.from("profile-pictures").getPublicUrl(filePath);
  if (urlError) {
